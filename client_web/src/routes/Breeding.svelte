...
<div class="grid">
  <section class="panel">
    ...
    {:else}
      <p class="dim small">
        Two different characters produce a third. Generation and inbreeding
        carry forward, so pairing close relatives has a real cost.
      </p>
    {/if}
    ...
    {#if preview.data}
      ...
      {#if !p.IsEligible}
        <p class="warn">{p.IneligibleReason || 'These two cannot breed.'}</p>
      {:else}
