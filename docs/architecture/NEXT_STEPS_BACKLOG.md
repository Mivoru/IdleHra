...
1. **Task 36 Phase 2: the wheel deals damage.** Plan: `docs/superpowers/plans/2026-09-24-task-36-world-boss-minigame.md`, Phase 2. Start on a fresh branch from `main`. Changes since the plan was written:
   - **DECIDED 2026-09-26 (spec section 3.3.1): a new weak plate for EVERY attempt, drawn only from the UNBROKEN plates, and armour that REGROWS every UTC midnight.** In `wheel` mode only.
     - The weak plate is held on the challenge (drawn with `RandomNumberGenerator` at issue), not on the snapshot.
     - `WeakPlateRevealed` is not used; four broken plates make the fifth certain.
     - LiveOps clears `BrokenPlateMask` at the same midnight reset that refills strikes.
     - The spec lists the tests.
     - Tuning lever, NOT decided: breaking only on a Seam, if the board strips too fast.
...
- **The item catalogue has HOLES.** Ids are positional (`_itemBaseIds[id - 1]`)
  and 161 entries were removed (111 in `d423b5b`, 40 legacy crafting
  materials in task 33, the last ten `*_crafting_material` ores the same
  day), so never renumber and never assume 1..N.
  `ItemCatalogueIntegrityTests` + `server/FolkIdle.Server.Tests/ItemIdLedger.txt`
  enforce it: no shifted id, no reused retired id, no reachable loot row or
  recipe naming a hole. A new item appends a line; a retired one becomes `-`.
- **52 of the 107 loot rows are dead padding** (indices 0-21, 47-76): no
  `_lootSegments` entry reaches them, and they cannot be deleted because
  segments slice `_lootEntries` by position. The integrity test pins the set.
