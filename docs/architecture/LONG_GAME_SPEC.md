...
3. **DONE - rings 2 and 3 of the tree, plus the respec gate.** Structure,
   ...
   - **ALL 15 nodes have their effect wired.** `EffectPending` is empty and
     `SkillNodeEffectTests` now asserts it stays that way, so a node cannot be
     quietly disabled instead of fixed.
