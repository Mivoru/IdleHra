...
# 1. Hide Raid completely
content = re.sub(r'<section class="panel">\s*<h2>Raid</h2>', r'<!-- Raid hidden -->\n    <section class="panel" style="display:none;">\n      <h2>Raid</h2>', content)

# 2. Fix isDonatableMaterial
old_isDonatable = r"  function isDonatableMaterial\(baseId: string\): boolean \{[\s\S]*?return valid\.some\(v => baseId\.includes\(v\)\) && \(baseId\.includes\('log'\) \|\| baseId\.includes\('twig'\) \|\| baseId\.includes\('ore'\) \|\| baseId\.includes\('bar'\) \|\| baseId\.includes\('chunk'\) \|\| baseId\.includes\('crystal'\)\);\n  \}"
...
content = re.sub(old_isDonatable, new_isDonatable, content, flags=re.DOTALL)

# 3. Create unified Treasury panel
# We will match from <section class="panel"> <h2>Depot</h2> down to <section class="panel"> <h2>Members</h2>
match = re.search(r'<section class="panel">\s*<h2>Depot</h2>.*?<section class="panel">\s*<h2>Members</h2>', content, re.DOTALL)
if not match:
    print("Could not find Depot to Members section!")
else:
    ...
    content = content[:match.start()] + unified_panel + content[match.end() - len('</section>\n      <h2>Members</h2>') + len('</section>\n') - 4:]
...
content = re.sub(r"const donateMax = \$derived\([\s\S]*?donateMaterial === 'gold'[\s\S]*?\);", 
